 ...
 :XML(XML_interface)//TODO: using one global copy may have problems.
{
  ...
  */
  ...
  if (procdynp.numGPU > 0) {
    ...
    area.set_area(area.get_area() + gpu->area.get_area());//placement and routing overhead is 10%, gpu scales worse than cache 40% is accumulated from 90 to 22nm
    ...
    for (i = 0;i < numSTLB; i++)
      {
        ...
        if (procdynp.homoSTLB){
          ...
          area.set_area(area.get_area() + stlb.area.get_area());//placement and routing overhead is 10%, stlb scales worse than cache 40% is accumulated from 90 to 22nm
...
//#define FINE_GRAINED 1
    ...
    energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areaiFRAT, tdpiFRAT, 0.0, lkgiFRAT));

    sprintf(name, "P(%d)_iRRAT",ii); 
    sprintf(conn, "P(%d)_IRF",ii); 
    energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areaiRRAT, tdpiRRAT, 0.0, lkgiRRAT));

    sprintf(name, "P(%d)_fFRAT",ii); 
    sprintf(conn, "P(%d)_IRF",ii); 
    energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areafFRAT, tdpfFRAT, 0.0, lkgfFRAT));

    sprintf(name, "P(%d)_fRRAT",ii); 
    sprintf(conn, "P(%d)_IRF",ii); 
    energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areafRRAT, tdpfRRAT, 0.0, lkgfRRAT));

    sprintf(name, "P(%d)_iFreeL",ii); 
    sprintf(conn, "P(%d)_IRF",ii); 
    energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areaifreeL, tdpifreeL, 0.0, lkgifreeL));

    sprintf(name, "P(%d)_fFreeL",ii); 
    sprintf(conn, "P(%d)_IRF",ii); 
    energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areaffreeL, tdpffreeL, 0.0, lkgffreeL));
    ...
    energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areaGlobalBPT, tdpGlobalBPT, 0.0, lkgGlobalBPT));

    sprintf(name, "P(%d)_localBPT",ii); 
    sprintf(conn, "P(%d)_icache", ii); 
    energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, arealocalBPT, tdplocalBPT, 0.0, lkglocalBPT));

    sprintf(name, "P(%d)_Chooser",ii); 
    sprintf(conn, "P(%d)_icache", ii); 
    energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areaChooser, tdpChooser, 0.0, lkgChooser));
    ...
    energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areaicache, tdpicache, 0.0, lkgicache));
    ...
    area = 1.25e-12*(pipeline +
        cores[i]->mmu->itlb->area.get_area()  +
        cores[i]->ifu->icache.area.get_area() +
        cores[i]->ifu->icachecc.area.get_area());
    tdp =  (cores[i]->mmu->itlb->power.readOp.dynamic  +
        cores[i]->ifu->icache.power.readOp.dynamic    +
        cores[i]->ifu->icachecc.power.readOp.dynamic);
    lkg = cores[i]->mmu->itlb->rt_power.readOp.get_leak()  + 
        cores[i]->ifu->icache.rt_power.readOp.get_leak()   +
    ...
    energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areadcache, tdpdcache, 0.0, lkgdcache));
    ...
    area = 1.25e-12*(pipeline +
        cores[i]->mmu->dtlb->area.get_area()  +
        cores[i]->lsu->dcache.area.get_area() +
        cores[i]->lsu->dcachecc.area.get_area());
    tdp = (cores[i]->mmu->dtlb->power.readOp.dynamic  +
        cores[i]->lsu->dcache.power.readOp.dynamic    +
        cores[i]->lsu->dcachecc.power.readOp.dynamic);
    lkg = cores[i]->mmu->dtlb->rt_power.readOp.get_leak()  +
        cores[i]->lsu->dcache.rt_power.readOp.get_leak()   +
    ...
    if (XML->sys.core[i].scoore) {
      ...
      energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areaVPCfilter, tdpVPCfilter, 0.0, lkgVPCfilter));

      sprintf(name, "P(%d)_vpc_buffer",ii); 
      sprintf(conn, "P(%d)_dcache",ii); 
      energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areavpc_buffer, tdpvpc_buffer, 0.0, lkgvpc_buffer));
    } else {
      if (XML->sys.core[i].machine_type==0) { // OoO
        ...
        sprintf(conn, "P(%d)_dcache",ii); 
        energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areaLSQ, tdpLSQ, 0.0, lkgLSQ));

        sprintf(name, "P(%d)_LoadQ",ii); 
        sprintf(conn, "P(%d)_dcache",ii); 
        energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areaLoadQ, tdpLoadQ, 0.0, lkgLoadQ));

        sprintf(name, "P(%d)_ssit",ii); 
        sprintf(conn, "P(%d)_dcache",ii); 
        energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areassit, tdpssit, 0.0, lkgssit));

        sprintf(name, "P(%d)_lfst",ii); 
        sprintf(conn, "P(%d)_dcache",ii); 
        energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, arealfst, tdplfst, 0.0, lkglfst));
      }else {
    ...
    if (XML->sys.core[i].scoore) {
      ...
      area = 1.25e-12*(pipeline +
          cores[i]->lsu->VPCfilter.area.get_area()        +
          cores[i]->lsu->vpc_buffer->area.get_area()); // +
      // cores[i]->lsu->strb->area.get_area()         +
      // cores[i]->lsu->ldrb->area.get_area())
      tdp = (cores[i]->lsu->VPCfilter.power.readOp.dynamic     +
          cores[i]->lsu->vpc_buffer->power.readOp.dynamic); // +
      ...
      lkg = cores[i]->lsu->VPCfilter.rt_power.readOp.get_leak()     +
          cores[i]->lsu->vpc_buffer->rt_power.readOp.get_leak(); // +
      ...
      if (XML->sys.core[i].machine_type==0) { // OoO
        ...
        area = 1.25e-12*(pipeline +
            cores[i]->lsu->LSQ->area.get_area()   +
            cores[i]->lsu->LoadQ->area.get_area() +
            cores[i]->lsu->ssit->area.get_area()  +
            cores[i]->lsu->lfst->area.get_area());
        tdp = (cores[i]->lsu->LSQ->power.readOp.dynamic   +
            cores[i]->lsu->LoadQ->power.readOp.dynamic +
            cores[i]->lsu->ssit->power.readOp.dynamic  +
            cores[i]->lsu->lfst->power.readOp.dynamic); 
        lkg = cores[i]->lsu->LSQ->rt_power.readOp.get_leak()   +
            cores[i]->lsu->LoadQ->rt_power.readOp.get_leak() +
            cores[i]->lsu->ssit->rt_power.readOp.get_leak()  +
            cores[i]->lsu->lfst->rt_power.readOp.get_leak(); 
    ...
    sprintf(conn, "P(%d)_IRF",ii); 
    energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, areaint_inst_window, tdpint_inst_window, 0.0, lkgint_inst_window));
    ...
    area = 1.5e-12*(pipeline + cores[i]->exu->rfu->IRF->area.get_area() + 
        pipeline + cores[i]->exu->rfu->FRF->area.get_area());  
    tdp = cores[i]->exu->rfu->IRF->power.readOp.dynamic + 
      cores[i]->exu->rfu->FRF->power.readOp.dynamic;  
    lkg = cores[i]->exu->rfu->IRF->rt_power.readOp.get_leak()+ 
      cores[i]->exu->rfu->FRF->rt_power.readOp.get_leak();  
    type = 0;
    energyBundle->cntrs.push_back(Container((const char *)name, (char *)conn, (char *)conn, type, area, tdp, 0.0, lkg));
    area = tdp = lkg = 0;
#endif
...
void Processor::dumpCoreDyn(int i) {
  ...
  set_pppm(pppm_t, (*clockInterval)[i]/XML->sys.core[0].pipeline_depth[0], 1/XML->sys.core[0].pipeline_depth[0], 1/XML->sys.core[0].pipeline_depth[0], 1/XML->sys.core[0].pipeline_depth[0]);
  ...
  dyn = // 3
    (((cores[i]->corepipe->power*pppm_t) +
      cores[i]->ifu->BPT->globalBPT->rt_power  +
      cores[i]->ifu->BPT->L1_localBPT->rt_power +
      cores[i]->ifu->BPT->chooser->rt_power     +
      cores[i]->ifu->BPT->RAS->rt_power         +
      cores[i]->ifu->BTB->rt_power              +
      cores[i]->ifu->IB->rt_power)*cccm_t).readOp.get_dyn();  
  ...
  dyn = // 4
    (((cores[i]->corepipe->power*pppm_t) +
      cores[i]->mmu->itlb->rt_power  +
      cores[i]->ifu->icache.rt_power +
      cores[i]->ifu->icachecc.rt_power)*cccm_t).readOp.get_dyn();
  sprintf(name, "P(%d)_icache",i); 
  energyBundle->setDynCycByName(name, dyn, cyc);
#endif
  ...
  // dcache + dtlb
  dyn = // 5
    (((cores[i]->corepipe->power*pppm_t) + cores[i]->mmu->dtlb->rt_power+ cores[i]->lsu->dcache.rt_power + cores[i]->lsu->dcachecc.rt_power)*cccm_t).readOp.get_dyn();
  sprintf(name, "P(%d)_dcache",i); 
  energyBundle->setDynCycByName(name, dyn, cyc);
#endif
  ...
  if (XML->sys.core[i].scoore)
  {
    dyn = (((cores[i]->corepipe->power*pppm_t)  +
          ...
          cores[i]->lsu->vpc_buffer->rt_power  +
          cores[i]->lsu->vpc_specbw->rt_power       +
          cores[i]->lsu->vpc_specbr->rt_power)*cccm_t).readOp.get_dyn();
  ...
  }else
  {
    ...
    }else 
    {
      dyn = 
        (((cores[i]->corepipe->power*pppm_t)  +
          cores[i]->lsu->LSQ->rt_power)*cccm_t).readOp.get_dyn();  
      sprintf(name, "P(%d)_LSQ",i); 
      energyBundle->setDynCycByName(name, dyn, cyc);
    }
  }
  ...
  }else
  {
    ...
    {                
      dyn = 
        (((cores[i]->corepipe->power*pppm_t) +
          cores[i]->lsu->LSQ->rt_power   +
          cores[i]->lsu->LoadQ->rt_power +
          cores[i]->lsu->ssit->rt_power  +
          cores[i]->lsu->lfst->rt_power)*cccm_t).readOp.get_dyn(); 
    }else 
    {
      dyn = 
        (((cores[i]->corepipe->power*pppm_t)  +
  ...
  dyn = 
    (((cores[i]->corepipe->power*pppm_t)  +
      cores[i]->exu->rfu->IRF->rt_power)*cccm_t).readOp.get_dyn() +
    (((cores[i]->corepipe->power*pppm_t) +
      cores[i]->exu->rfu->FRF->rt_power)*cccm_t).readOp.get_dyn();  
  ...
  if (XML->sys.core[i].scoore){                            
    dyn = 
      (((cores[i]->corepipe->power*pppm_t) + cores[i]->lsu->VPCfilter.rt_power)*cccm_t).readOp.get_dyn();
...
void Processor::dumpGPUDyn(int i) {
  ...
  if ( XML->sys.gpu.SMS[i].lanes[0].total_cycles>= 100)
    set_pppm(cccm_t, double(POW_SCALE_DYN*energyBundle->getFreq()/(double (XML->sys.gpu.SMS[i].lanes[0].total_cycles))), 1,1,1);
  else
    set_pppm(cccm_t, 0, 1,1,1);
