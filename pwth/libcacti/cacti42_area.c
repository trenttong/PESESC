...
/* added by Peter Schultz, 8 April 2010, to combat uninitialized members
   of the area_type structure */

static void
init_area_type (area_type * t)
{
  t->width = t->height = t->scaled_area = 0;
}
...
subarraymem_area (int C, int B, int A, int Ndbl, int Ndwl, double Nspd, int RWP, int ERP, int EWP, int NSER, double techscaling_factor)	/* returns area of subarray */
{
  ...
  memarea.height =
    ceil ((double) (noof_rows) / 16.0) * stitch_ramv + (BitHeight1x1 +
							Widthtrack * 2 *
							(RWP + ERP + EWP -
							 1)) * noof_rows;
  memarea.width =
    noof_colns * (BitWidth1x1 +
...
decodemem_row (int C, int B, int A, int Ndbl, double Nspd, int Ndwl, int RWP, int ERP, int EWP)	/* returns area of post decode */
{
  ...
  init_area_type (&decinv);
  init_area_type (&worddriveinv);
  init_area_type (&postdecodearea);
...
/* this puts the different predecode blocks for the different ports side by side and does not put them as an array or something */
{
  ...
  int N3to8;
  init_area_type (&predecode);
  init_area_type (&predecode_temp);
...
postdecode_area (int noof_rows, int RWP, int ERP, int EWP)
{
  ...
  init_area_type (&postdecode);
  init_area_type (&decinverter);
  ...
  if (noof_rows < 1)
    ...
    numstack =
      (int) (ceil ((1.0 / 3.0) * logtwo_area ((double) (noof_rows))));
  if (numstack == 0)
    numstack = 1;
  if (numstack > 5)
    numstack = 5;
...
colmux (int Ndbl, double Nspd, int RWP, int ERP, int EWP, int NSER)	/* gives the height of the colmux */
{
  ...
  init_area_type (&colmux_area);
  colmux_area.height = (2 * Wiso + 3 * (2 * Widthcontact + 1 / FUDGEFACTOR)) * (RWP + ERP + EWP);	//Shyam: Need to understand what the +1 is for
  colmux_area.width =
...
precharge (int Ndbl, double Nspd, int RWP, int ERP, int EWP, int NSER)
{
  ...
  init_area_type (&precharge_area);
  if (Ndbl * Nspd > 1)
    {
      precharge_area.height =
	(Wbitpreequ + 2 * Wbitdropv + Wwrite +
	 2 * (2 * Widthcontact + 1 / FUDGEFACTOR) +
...
area_type
subarraytag_area (int baddr, int C, int B, int A, int Ntdbl, int Ntdwl, int Ntspd, double NSubbanks, int RWP, int ERP, int EWP, int NSER, double techscaling_factor)	/* returns area of subarray */
{
  area_type tagarea;
  int noof_rows, noof_colns, Tagbits;
  int conservative_NSER;
  init_area_type (&tagarea);
  ...
  tagarea.width =
    noof_colns * (BitWidth1x1 +
		  (Widthtrack * 2 *
		   (RWP + (ERP - conservative_NSER) + EWP - 1) +
...
decodetag_row (int baddr, int C, int B, int A, int Ntdbl, int Ntspd, int Ntdwl, double NSubbanks, int RWP, int ERP, int EWP)	/* returns area of post decode */
{
  ...
  Tagbits =
    (int) (baddr - logtwo_area ((double) (C)) + logtwo_area ((double) (A)) +
...
muxdriverdecode (int B, int b0, int RWP, int ERP, int EWP)
{
  ...
  init_area_type (&muxdrvdecode_area);
  init_area_type (&predecode);
  init_area_type (&postdecode);
  noof_rows = (8 * B) / b0;
...
muxdrvsig (int A, int B, int b0)	/* generates the 8B/b0*A signals */
{
  ...
  init_area_type (&outdrvsig_area);
  init_area_type (&muxdrvsig_area);
  noof_rows = (8 * B) / b0;
...
area_type
datasubarray (int C, int B, int A, int Ndbl, int Ndwl, double Nspd, int RWP,
	      int ERP, int EWP, int NSER, double techscaling_factor)
{
  ...
  init_area_type (&datasubarray_area);
  init_area_type (&mem_area);
  init_area_type (&postdecode_area);
  init_area_type (&colmux_area);
  init_area_type (&precharge_area);
  init_area_type (&senseamp_area);
  mem_area =
...
area_type
datasubblock (int C, int B, int A, int Ndbl, int Ndwl, double Nspd, int SB,
	      ...
	      double techscaling_factor)
{
  ...
  init_area_type (&datasubarray_area);
  init_area_type (&datasubblock_area);
  SB_ = SB;
  if (SB_ == 0)
    {
      SB_ = 1;
...
area_type
dataarray (int C, int B, int A, int Ndbl, int Ndwl, double Nspd, int b0,
	   int RWP, int ERP, int EWP, int NSER, double techscaling_factor)
{
  ...
  //techscaling_factor) * CONVERT_TO_MMSQUARE;
  area_all_datasubarrays =
    Ndbl * Ndwl * datasubarray_area.height * datasubarray_area.width *
    CONVERT_TO_MMSQUARE;
...
area_type
tagsubarray (int baddr, int C, int B, int A, int Ndbl, int Ndwl, double Nspd,
	     double NSubbanks, int RWP, int ERP, int EWP, int NSER,
	     double techscaling_factor)
{
  int conservative_NSER;
  area_type tagsubarray_area, tag_area, postdecode_area, colmux_area,
    precharge_area, senseamp_area, comp_area;
  init_area_type (&tagsubarray_area);
  init_area_type (&tag_area);
  init_area_type (&postdecode_area);
  init_area_type (&colmux_area);
  init_area_type (&precharge_area);
  init_area_type (&senseamp_area);
  init_area_type (&comp_area);
...
area_type
tagsubblock (int baddr, int C, int B, int A, int Ndbl, int Ndwl, double Nspd,
	     double NSubbanks, int SB, int RWP, int ERP, int EWP, int NSER,
	     double techscaling_factor)
{
  int N3to8, T;
  int SB_;
  ...
  int conservative_NSER;
  init_area_type (&tagsubarray_area);
  init_area_type (&tagsubblock_area);
...
area_type
tagarray (int baddr, int C, int B, int A, int Ndbl, int Ndwl, double Nspd,
	  double NSubbanks, int RWP, int ERP, int EWP, int NSER,
	  double techscaling_factor)
{
  //int SB, CSB, N3to8, T;
  ...
  //techscaling_factor) * CONVERT_TO_MMSQUARE;
  area_all_tagsubarrays =
    Ndbl * Ndwl * tagsubarray_area.height * tagsubarray_area.width *
    CONVERT_TO_MMSQUARE;
...
void
area (int baddr, int b0, int Ndbl, int Ndwl, double Nspd, int Ntbl, int Ntwl,
      int Ntspd, double NSubbanks, parameter_type * parameters,
      arearesult_type * result)
{
  ...
  rows_datasubarray =
    (parameters->cache_size /
     ((parameters->block_size) * (parameters->data_associativity) * Ndbl *
      Nspd));
  colns_datasubarray = (Ndbl * Nspd);
  rows_tagsubarray =
    (parameters->cache_size /
     ((parameters->block_size) * (parameters->tag_associativity) * Ntbl *
      Ntspd));
  ...
  result->totalarea =
    ...
    (parameters->num_readwrite_ports +
     parameters->num_write_ports) * result->datawritesig_area.height *
    result->datawritesig_area.width +
    ...
    (parameters->num_readwrite_ports +
     parameters->num_read_ports) * result->tagoutdrvsig_area.height *
    result->tagoutdrvsig_area.width;
...
fadecode_row (int C, int B, int Ndbl, int RWP, int ERP, int EWP)	/*returns area of post decode */
{
  ...
  numstack =
    (int) (ceil
	   ((1.0 / 3.0) *
	    logtwo_area ((double) ((double) C / (double) (B)))));
  if (numstack == 0)
    numstack = 1;
  if (numstack > 6)
    numstack = 6;
...
fasubarray (int baddr, int C, int B, int Ndbl, int RWP, int ERP, int EWP, int NSER, double techscaling_factor)	/* returns area of subarray */
{
  ...
  init_area_type (&FAarea);
  init_area_type (&fadecoderow);
  init_area_type (&faramcell);
  ...
  precharge =
    Wbitpreequ + 2 * Wbitdropv + Wwrite + 2 * (2 * Widthcontact +
					       1 / FUDGEFACTOR) +
    3 * Widthptondiff;
   ...
   */
  faramcell.width =
    noof_colnsdata * (BitWidth1x1 +
		      (Widthtrack * 2 * (RWP + (ERP - NSER) + EWP - 1) +
		       Widthtrack * NSER)) +
    2 * (HTagbits * ((CAM2x2Width_1p + 2 * widthoverhead) - Widthcontact)) +
    (BitWidth + Widthtrack * 2 * (RWP + ERP + EWP - 1)) + (FArowNANDWidth +
							   FArowNOR_INVWidth)
    * (RWP + ERP + EWP);
...
area_type
faarea (int baddr, int b0, int C, int B, int Ndbl, int RWP, int ERP, int EWP,
	int NSER, double techscaling_factor)
{
  ...
  init_area_type (&fasubarray_area);
  init_area_type (&fa_area);
  init_area_type (&temp);
...
void
fatotalarea (int baddr, int b0, int Ndbl, parameter_type * parameters,
...
void
area_subbanked (int baddr, int b0, int RWP, int ERP, int EWP, int Ndbl,
		...
		area_type * result_subbanked, arearesult_type * result)
{
  ...
  if (NSubbanks == 8)
	...
	(result_area.dataarray_area.width +
	 result_area.tagarray_area.width) * 2 + (ADDRESS_BITS +
						 BITOUT) * WIREPITCH *
	NSubbanks * (RWP + ERP + EWP) * 0.5;
      total_width =
	2 * (2 * result_area.dataarray_area.height +
	     2 * (RWP + ERP + EWP) * ADDRESS_BITS * WIREPITCH) +
	(ADDRESS_BITS + BITOUT) * WIREPITCH * NSubbanks * (RWP + ERP + EWP);
    }
  ...
  if (NSubbanks > 8)
	...
	((result_area.
	 ...
	 (ADDRESS_BITS + BITOUT) * WIREPITCH * 16 * (RWP + ERP +
						     EWP) * 0.25) +
      ...
      else
	{
	  for (i = 1; i <= iter_height; i++)
		...
		2 * (inter_height) + (ADDRESS_BITS +
				      BITOUT) * WIREPITCH * 4 *
		inter_subbanks * (RWP + ERP + EWP) * 0.5;
	      inter_height = total_height;
      ...
      else
	{
	  if (htree % 2 == 0)
	      ...
	      for (i = 1; i <= iter_width_double; i++)
		{
	      ...
	      total_width +=
		(ADDRESS_BITS + BITOUT) * WIREPITCH * (RWP + ERP +
						       EWP) * NSubbanks / 2;
	    }
	}
    }
...
int
data_organizational_parameters_valid (int B, int A, int C, int Ndwl, int Ndbl,
				      double Nspd, char assoc,
				      double NSubbanks)
{
...
int
tag_organizational_parameters_valid (int B, int A, int C, int Ntwl, int Ntbl,
