...
double
gatecap (double width, double wirelength)	/* returns gate capacitance in Farads */
...
double
gatecappass (double width, double wirelength)	/* returns gate capacitance in Farads */
     /* width: gate width in um (length is Leff) */
     /* wirelength: poly wire length going to gate in lambda */
{
...
double
draincap (double width, int nchannel, int stack)	/* returns drain cap in Farads */
	 /* width: in um */
     ...
     /* stack: number of transistors in series that are on */
{
  ...
  if (stack > 1)
	...
	{
	  cap =
	    3.0 * Leff * width / 2.0 * Cdiffarea + 6.0 * Leff * Cdiffside +
	    width * Coverlap;
	  cap +=
	...
	{
	  cap =
	    3.0 * Leff * width * Cdiffarea + (6.0 * Leff +
					      width) * Cdiffside +
	    width * Coverlap;
	  cap +=
	    (double) (stack - 1) * (Leff * width * Cdiffarea +
...
double
transresswitch (double width, int nchannel, int stack)	/* returns resistance in ohms */
     /* width: in um */
     /* nchannel: whether n or p-channel (boolean) */
...
double
transreson (double width, int nchannel, int stack)	/* returns resistance in ohms */
     /* width: in um */
     /* nchannel: whether n or p-channel (boolean) */
     /* stack: number of transistors in series */
{
...
double
restowidth (double res, int nchannel)	/* returns width in um */
...
double
horowitz (double inputramptime, double tf, double vs1, double vs2, int rise)
