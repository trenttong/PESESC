...
void
output_time_components (result_type * result, parameter_type * parameters)
{
  int A;
...
void
output_area_components (arearesult_type * arearesult,
...
void
output_data (result_type * result, arearesult_type * arearesult,
	     parameter_type * parameters)
{
  ...
  datapath =
    result->subbank_address_routing_delay + result->decoder_delay_data +
    result->wordline_delay_data + result->bitline_delay_data +
    result->sense_amp_delay_data + result->total_out_driver_delay_data +
    result->data_output_delay;
  if (parameters->tag_associativity == 1)
    {
      tagpath =
	result->subbank_address_routing_delay + result->decoder_delay_tag +
	result->wordline_delay_tag + result->bitline_delay_tag +
	result->sense_amp_delay_tag + result->compare_part_delay +
...
#  else
  printf
    ("%d %d %d  %d %d %d %d %d %d  %e %e %e %e  %e %e %e %e  %e %e %e %e  %e %e %e %e  %e %e\n",
     ...
     result->compare_part_delay, result->drive_mux_delay, result->selb_delay,
     result->drive_valid_delay, result->data_output_delay,
...
void
output_params (parameter_type * parameters)
{
  printf ("\n***** Cache Parameters:\n");
  printf ("  Cache Size: %d\n", (int) (parameters->cache_size));
  printf ("  Number of sets per bank: %d\n", parameters->number_of_sets);
