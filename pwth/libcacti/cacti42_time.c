...
void
compute_device_widths (int C, int B, int A, int fullyassoc, int Ndwl,
		       int Ndbl, double Nspd)
{
  ...
  double previous_ndriveW = 0, previous_pdriveW = 0, current_ndriveW =
    0, current_pdriveW = 0;
  int i;
  ...
  Cline =
    (gatecappass (Wmemcella, (BitWidth - 2 * Wmemcella) / 2.0) +
     gatecappass (Wmemcella,
		  (BitWidth - 2 * Wmemcella) / 2.0) + Cwordmetal) * cols;
  ...
  Cline =
    gatecap (WdecNORn + WdecNORp,
	     ((numstack * 40 / FUDGEFACTOR) +
	      20.0 / FUDGEFACTOR)) * rows / 8 +
    GlobalCbitmetal * (l_predec_nor_v) + GlobalCwordmetal * (l_predec_nor_h);
  ...
  if (Ndwl * Ndbl == 1)
    {
      ...
      Cline =
	4 * gatecap (previous_ndriveW + previous_pdriveW,
		     10.0 / FUDGEFACTOR) + GlobalCwordmetal * horizontal_edge;
      Cload = Cline / gatecap (1.0, 0.0);
  ...
  else
    {
      ...
      Cline =
	4 * gatecap (previous_ndriveW + previous_pdriveW,
		     10.0 / FUDGEFACTOR) + GlobalCwordmetal * horizontal_step;
      Cload = Cline / gatecap (1.0, 0.0);
      ...
      while (nr_subarrays_left > 1)
	{
	  ...
	  if (v_or_h)
		...
		0.5 * GlobalRbitmetal * vertical_step;
	      Cline =
		gatecap (previous_ndriveW + previous_pdriveW,
			 0) + GlobalCbitmetal * vertical_step;
	      v_or_h = 0;
	      vertical_step *= 2;
	      nr_subarrays_left /= 2;
	    }
		...
		0.5 * GlobalRwordmetal * horizontal_step;
	      Cline =
		gatecap (previous_ndriveW + previous_pdriveW,
			 0) + GlobalCwordmetal * horizontal_step;
	      v_or_h = 1;
	      horizontal_step *= 2;
	      nr_subarrays_left /= 2;
	    }
	...
	{
	  printf
	    ("Too many segments in the data decoder H-tree. Overflowing the preallocated array!");
	  exit (1);
	}
      wire_cap =
	GlobalCbitmetal * v_inv_predecode +
	GlobalCwordmetal * h_inv_predecode;
      wire_res =
	0.5 * (GlobalRbitmetal * v_inv_predecode +
  ...
  Wdecdriven_first =
    (Wdecdriven_second + Wdecdrivep_second) * SizingRatio / 3;
  Wdecdrivep_first =
  ...
  if (Ndwl * Ndbl == 1)
    {
      ...
      Cline =
	gatecap (Wsenseextdrv1n + Wsenseextdrv1p,
		 10.0 / FUDGEFACTOR) + GlobalCwordmetal * cols;
      Cload = Cline / gatecap (1.0, 0.0);
  ...
  else if (Ndwl * Ndbl == 2)
    {
      ...
      Cline =
	gatecap (Wsenseextdrv1n + Wsenseextdrv1p,
		 10.0 / FUDGEFACTOR) + GlobalCwordmetal * 2 * cols;
      Cload = Cline / gatecap (1.0, 0.0);
  ...
  if (nr_outdrvtreesegments >= 20)
    {
      printf
	("Too many segments in the output H-tree. Overflowing the preallocated array!");
      exit (1);
    }
  // typical width of gate considered for the estimating val of draincap.
  Cload =
    gatecap (previous_ndriveW + previous_pdriveW,
	     0) + Coutdrvtreesegments[0] + (draincap (5.0 / FUDGEFACTOR, NCH,
						      1) +
					    draincap (5.0 / FUDGEFACTOR, PCH,
						      1)) * A * muxover;
...
void
compute_tag_device_widths (int C, int B, int A, int Ntspd, int Ntwl, int Ntbl,
			   double NSubbanks)
{
  ...
  numstack =
    (int) ceil ((1.0 / 3.0) *
		logtwo ((double)
			((double) C / (double) (B * A * Ntbl * Ntspd))));
  if (numstack == 0)
    numstack = 1;
  if (numstack > 5)
    numstack = 5;
  ...
  if (Ntwl * Ntbl == 1)
	...
	{
	  printf
	    ("Too many segments in the tag decoder H-tree. Overflowing the preallocated array!");
	  exit (1);
	}
  ...
  Cline =
    4 * gatecap (Wtdec3to8n + Wtdec3to8p, 10.0 / FUDGEFACTOR) + wire_cap;
  Cload = Cline / gatecap (1.0, 0.0);
...
double
cmos_ileakage (double nWidth, double pWidth,
	       double nVthresh_dual, double nVthreshold, double pVthresh_dual,
	       double pVthreshold)
{
  double leakage = 0.0;
  ...
  if (have_leakage_params)
      ...
      if (dualVt == TRUE)
	{
	  ...
	  if ((cached_nmos_thresh == nVthresh_dual)
	    ...
	    {
	      leakage =
		nWidth * norm_nmos_leakage + pWidth * norm_pmos_leakage;
	    }
	  else
	    {
	      leakage =
		simplified_cmos_leakage (nWidth * inv_Leff, pWidth * inv_Leff,
					 nVthresh_dual, pVthresh_dual,
					 &norm_nmos_leakage,
					 &norm_pmos_leakage);
...
void
mult_powerDef (powerDef * power, int val)
{
...
void
mac_powerDef (powerDef * sum, powerDef * mult, int val)
{
...
void
copy_powerDef (powerDef * dest, powerDef source)
{
...
void
copy_and_div_powerDef (powerDef * dest, powerDef source, double val)
{
...
void
add_powerDef (powerDef * sum, powerDef a, powerDef b)
{
...
double
objective_function (double delay_weight, double area_weight,
		    double power_weight, double delay, double area,
		    double power)
{
  return
    (double) (area_weight * area + delay_weight * delay +
	      power_weight * power);
}
...
void
subbank_routing_length (int C, int B, int A,
			char fullyassoc,
			int Ndbl, double Nspd, int Ndwl, int Ntbl, int Ntwl,
...
void
subbanks_routing_power (char fullyassoc,
			int A,
			double NSubbanks,
			...
			powerDef * power)
{
  ...
  if (NSubbanks == 8.0)
    {
      wire_cap =
	GlobalCbitmetal * (*subbank_v) + GlobalCwordmetal * (*subbank_h);
      /* buffer stage */
      Ceq = draincap (Wdecdrivep, PCH, 1) + draincap (Wdecdriven, NCH, 1) +
	gatecap (Wdecdrivep + Wdecdriven, 0.0);
  ...
  if (NSubbanks > 8.0)
      ...
      for (i = 1; i <= blocks; i++)
	{
	  ...
	  /* calculation of out driver power */
	  wire_cap_outdrv =
	    line_v * GlobalCbitmetal + line_h * GlobalCwordmetal;
...
double
address_routing_delay (int C, int B, int A,
		       char fullyassoc,
		       int Ndwl, int Ndbl, double Nspd, int Ntwl, int Ntbl,
		       ...
		       powerDef * power)
{
  ...
  Cline =
    *NSubbanks * (gatecap (Wdecdrivep_first + Wdecdriven_first, 0.0) +
		  gatecap (Wtdecdrivep_first + Wtdecdriven_first,
			   0.0)) + wire_cap;
  Cload = Cline / gatecap (1.0, 0.0);
  ...
  nextinputtime =
    horowitz (nextinputtime, tf, VTHINV360x240, VTHINV360x240,
	      FALL) / (VTHINV360x240);

  dynEnergy = ADDRESS_BITS * Ceq * .5 * VddPow * VddPow;

  lkgCurrent +=
    ADDRESS_BITS * 0.5 * cmos_ileakage (Waddrdrvn2, Waddrdrvp2,
					Vt_bit_nmos_low, Vthn,
					Vt_bit_pmos_low, Vthp);

  Ceq =
    draincap (Waddrdrvp1, PCH, 1) + draincap (Waddrdrvn1, NCH,
					      1) + wire_cap +
    *NSubbanks * (gatecap (Wdecdrivep_first + Wdecdriven_first, 0.0) +
  ...
  lkgCurrent +=
    ADDRESS_BITS * 0.5 * cmos_ileakage (Waddrdrvn1, Waddrdrvp1, Vthn,
					Vt_bit_nmos_low, Vt_bit_pmos_low,
					Vthp);
...
double
decoder_delay (int C, int B, int A, int Ndwl, int Ndbl, double Nspd,
	       ...
	       int *nor_inputs, powerDef * power)
{
  ...
  addr_bits_routed =
    (int) (logtwo ((double) ((double) C / (double) B)) + EPSILON);
  ...
  if (NSubbanks > 2)
    ...
    {
      Ceq =
	draincap (Wdecdrivep_first, PCH, 1) + draincap (Wdecdriven_first, NCH,
							1) +
	gatecap (Wdecdrivep_first + Wdecdriven_first, 0.0);
  ...
  lkgCurrent +=
    addr_bits_routed * 0.5 * cmos_ileakage (Wdecdriven_first,
					    Wdecdrivep_first, Vt_bit_nmos_low,
					    Vthn, Vt_bit_pmos_low,
					    Vthp) * 1.0 / (Ndwl * Ndbl);
  ...
  *Tdecdrive += this_delay;
  ...
  if (nr_dectreesegments)
    {
      Ceq =
	draincap (Wdecdrivep_second, PCH, 1) + draincap (Wdecdriven_second,
							 NCH,
							 1) +
	gatecap (3 * WdecdrivetreeN[nr_dectreesegments - 1],
		 0) + Cdectreesegments[nr_dectreesegments - 1];
      Req =
	transreson (Wdecdriven_second, NCH,
		    1) + Rdectreesegments[nr_dectreesegments - 1];

      tf = Ceq * Req;
      this_delay =
	horowitz (inrisetime, tf, VTHINV360x240, VTHINV360x240, RISE);
      ...
      *Tdecdrive += this_delay;
      ...
      lkgCurrent +=
	addr_bits_routed * 0.5 * cmos_ileakage (Wdecdriven_second,
						Wdecdrivep_second,
						Vt_bit_nmos_low, Vthn,
						Vt_bit_pmos_low,
						Vthp) * 1.0 / (Ndwl * Ndbl);
  ...
  for (i = nr_dectreesegments; i > 2; i--)
    {
      ...
      Ceq =
	(Cdectreesegments[i - 2] +
	 draincap (2 * WdecdrivetreeN[i - 1], PCH,
		   1) + draincap (WdecdrivetreeN[i - 1], NCH,
				  1) + gatecap (3 * WdecdrivetreeN[i - 2],
						0.0));
      Req =
	(Rdectreesegments[i - 2] +
	 transreson (WdecdrivetreeN[i - 1], NCH, 1));
      tf = Req * Ceq;
  ...
  if (nr_dectreesegments)
    {
      Ceq =
	4 * gatecap (Wdec3to8n + Wdec3to8p,
		     10.0 / FUDGEFACTOR) + Cdectreesegments[0] +
	draincap (2 * WdecdrivetreeN[0], PCH,
		  1) + draincap (WdecdrivetreeN[0], NCH, 1);
      ...
      lkgCurrent +=
	1.0 / (Ndwl * Ndbl) * pow (2,
				   nr_dectreesegments) * addr_bits_routed *
	0.5 * cmos_ileakage (WdecdrivetreeN[0], 2 * WdecdrivetreeN[0],
			     Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp);
    }
  else
    {
	...
	draincap (Wdecdrivep_second, PCH, 1) + draincap (Wdecdriven_second,
      ...
      dynPower += addr_bits_routed * Ceq * .5 * VddPow * VddPow;
      lkgCurrent +=
	1.0 / (Ndwl * Ndbl) * addr_bits_routed * 0.5 *
	cmos_ileakage (Wdecdriven_second, Wdecdrivep_second, Vt_bit_nmos_low,
  ...
  *Tdecdrive += this_delay;
  ...
  lkgCurrent +=
    8 * 0.2 * 0.5 * cmos_ileakage (Wdec3to8n, Wdec3to8p, Vt_bit_nmos_low,
				   Vthn, Vt_bit_pmos_low, Vthp) *
  ...
  Ceq = 3 * draincap (Wdec3to8p, PCH, 1) + draincap (Wdec3to8n, NCH, 3) +
    ...
    GlobalCbitmetal * (l_predec_nor_v) + GlobalCwordmetal * (l_predec_nor_h);
  Rwire =
    GlobalRbitmetal * (l_predec_nor_v) / 2 +
  ...
  lkgCurrent +=
    0.5 * 0.2 * rows * cmos_ileakage (WdecNORn, WdecNORp, Vt_bit_nmos_low,
				      Vthn, Vt_bit_pmos_low, Vthp);
  ...
  dynPower +=
    0.5 * Nact * Ceq * VddPow * VddPow * 4 * ceil ((1.0 / 3.0) *
						   logtwo ((double)
							   ((double) C /
							    (double) (B * A *
								      Ndbl *
  ...
  *Tdecoder1 = horowitz (nextinputtime, tf, VTHNAND60x120, vth, RISE);
  ...
  Ceq = (gatecap (Wdecinvn + Wdecinvp, 20.0 / FUDGEFACTOR) +
	 numstack * draincap (WdecNORn, NCH, 1) + draincap (WdecNORp, PCH,
							    numstack));


  lkgCurrent +=
    0.5 * rows * cmos_ileakage (Wdecinvn, Wdecinvp, Vt_bit_nmos_low, Vthn,
				Vt_bit_pmos_low, Vthp);
  tf = Req * Ceq;
...
double
decoder_tag_delay (int C, int B, int A, int Ntwl, int Ntbl, int Ntspd,
		   double NSubbanks, double *Tdecdrive, double *Tdecoder1,
		   double *Tdecoder2, double inrisetime, double *outrisetime,
		   int *nor_inputs, powerDef * power)
{
  ...
  // Peter Schultz on 8 April 2010, unused? commented out.
  ...
  lkgCurrent =
    0.5 * cmos_ileakage (Wtdecdriven_first, Wtdecdrivep_first,
			 Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low,
			 Vthp) * 1.0 / (Ntwl * Ntbl);
  ...
  *Tdecdrive = 0;
  ...
  /*dt: the first inverter driving a bigger inverter */
  Ceq =
    draincap (Wtdecdrivep_first, PCH, 1) + draincap (Wtdecdriven_first, NCH,
						     1) +
  ...
  *Tdecdrive += this_delay;
  ...
  if (nr_tdectreesegments)
    {
      Ceq =
	draincap (Wtdecdrivep_second, PCH, 1) + draincap (Wtdecdriven_second,
							  NCH,
							  1) +
	gatecap (3 * WtdecdrivetreeN[nr_tdectreesegments - 1],
		 0) + Ctdectreesegments[nr_tdectreesegments - 1];
      Req =
	transreson (Wtdecdriven_second, NCH,
		    1) + Rtdectreesegments[nr_tdectreesegments - 1];
	...
	horowitz (inrisetime, tf, VTHINV360x240, VTHINV360x240, RISE);
      ...
      *Tdecdrive += this_delay;
      ...
      lkgCurrent +=
	tag_bits_routed * 0.5 * cmos_ileakage (Wtdecdriven_second,
					       Wtdecdrivep_second,
					       Vt_bit_nmos_low, Vthn,
					       Vt_bit_pmos_low,
					       Vthp) * 1.0 / (Ntwl * Ntbl);
  ...
  for (i = nr_tdectreesegments; i > 2; i--)
    {
      /*dt: this too should alternate... */
      Ceq =
	(Ctdectreesegments[i - 2] +
	 draincap (2 * WtdecdrivetreeN[i - 1], PCH,
		   1) + draincap (WtdecdrivetreeN[i - 1], NCH,
				  1) + gatecap (3 * WtdecdrivetreeN[i - 2],
						0.0));
      Req =
	(Rtdectreesegments[i - 2] +
	 transreson (WtdecdrivetreeN[i - 1], NCH, 1));
      tf = Req * Ceq;
      /*dt: This shouldn't be all falling, but interleaved. Have to fix that at some point. */
      this_delay =
	horowitz (inrisetime, tf, VTHINV360x240, VTHINV360x240, RISE);
  ...
  if (nr_tdectreesegments)
    {
      ...
      Ceq =
	4 * gatecap (Wtdec3to8n + Wtdec3to8p,
		     10.0 / FUDGEFACTOR) + Ctdectreesegments[0] +
	draincap (2 * WtdecdrivetreeN[0], PCH,
		  1) + draincap (WtdecdrivetreeN[0], NCH, 1);
      ...
      lkgCurrent +=
	1.0 / (Ntwl * Ntbl) * pow (2,
				   nr_tdectreesegments) * tag_bits_routed *
	0.5 * cmos_ileakage (WtdecdrivetreeN[0], 2 * WtdecdrivetreeN[0],
			     Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp);
    }
  else
    {
	...
	draincap (Wtdecdrivep_second, PCH, 1) + draincap (Wtdecdriven_second,
      ...
      dynPower += tag_bits_routed * Ceq * .5 * VddPow * VddPow;
      lkgCurrent +=
	1.0 / (Ntwl * Ntbl) * tag_bits_routed * 0.5 *
  ...
  *Tdecdrive += this_delay;
  ...
  lkgCurrent +=
    8 * 0.2 * 0.5 * cmos_ileakage (Wtdec3to8n, Wtdec3to8p, Vt_bit_nmos_low,
				   Vthn, Vt_bit_pmos_low, Vthp) *
    // For the all the 3-8 decoders per quad:
    ceil ((1.0 / 3.0) *
  ...
  Ceq = 3 * draincap (Wtdec3to8p, PCH, 1) + draincap (Wtdec3to8n, NCH, 3) +
    gatecap (WtdecNORn + WtdecNORp,
	     ((numstack * 40) / FUDGEFACTOR + 20.0 / FUDGEFACTOR)) * rows +
    GlobalCbitmetal * (l_predec_nor_v) + GlobalCwordmetal * (l_predec_nor_h);
  ...
  lkgCurrent +=
    0.5 * 0.2 * rows * cmos_ileakage (WtdecNORn, WtdecNORp, Vt_bit_nmos_low,
				      Vthn, Vt_bit_pmos_low, Vthp);
  ...
  *Tdecoder1 = horowitz (nextinputtime, tf, VTHNAND60x120, vth, RISE);
  ...
  Ceq =
    (gatecap (Wtdecinvn + Wtdecinvp, 20.0 / FUDGEFACTOR) +
     numstack * draincap (WtdecNORn, NCH, 1) + draincap (WtdecNORp, PCH,
							 numstack));
  ...
  *nor_inputs = numstack;
  ...
  lkgCurrent +=
    0.5 * rows * cmos_ileakage (Wtdecinvn, Wtdecinvp, Vt_bit_nmos_low, Vthn,
				Vt_bit_pmos_low, Vthp);
...
double
fa_tag_delay (int C, int B, int Ntwl, int Ntbl, int Ntspd,
	      ...
	      int *nor_inputs, powerDef * power)
{
	  ...
	  Ceq =
	    ...
	    gatecap (Wfadecdrive1p + Wfadecdrive1n,
		     10.0 / FUDGEFACTOR) * 2 + +FACwordmetal * sqrt ((rows +
								      1) *
								     Ntbl) /
	    2 + FACbitmetal * sqrt ((rows + 1) * Ntbl) / 2;
	  Rwire =
  ...
  Ceq = draincap (Wfadecdrivep, PCH, 2) + draincap (Wfadecdriven, NCH, 2)
    ...
    + gatecap (Wdummyn,
	       10.0 / FUDGEFACTOR) * (rows + 1) + FACbitmetal * (rows + 1);
  ...
  *Tagdrive =
  ...
  Ceq = .5 * ADDRESS_BITS * 2 * draincap (Wdummyn, NCH, 2)
    ...
    + gatecap (Waddrnandn + Waddrnandp,
	       10.0 / FUDGEFACTOR) + FACwordmetal * ADDRESS_BITS;
  ...
  *outrisetime = *Tag3 / VTHFA5;
  ...
  Ceq = draincap (Wfanorp, PCH, 2)
    + 2 * draincap (Wfanorn, NCH, 1) + gatecap (Wfainvn + Wfainvp,
						10.0 / FUDGEFACTOR);
  tf =
...
double
wordline_delay (int C, int B, int A, int Ndwl, int Ndbl, double Nspd,
		double inrisetime, double *outrisetime, powerDef * power)
{
  ...
  lkgCurrent =
    0.5 * cmos_ileakage (WwlDrvn, WwlDrvp, Vt_bit_nmos_low, Vthn,
			 Vt_bit_pmos_low, Vthp);
...
/*dt: incorporated leakage calc from eCacti*/
double
wordline_tag_delay (int C, int B, int A,
		    ...
		    double inrisetime, double *outrisetime, powerDef * power)
{
  ...
  lkgCurrent =
    0.5 * cmos_ileakage (WtwlDrvn, WtwlDrvp, Vt_bit_nmos_low, Vthn,
			 Vt_bit_pmos_low, Vthp);
...
double
bitline_delay (int C, int A, int B, int Ndwl, int Ndbl, double Nspd,
	       double inrisetime,
	       double *outrisetime, powerDef * power, double Tpre)
{
  ...
  Iport =
    cmos_ileakage (Wmemcella, 0, Vt_cell_nmos_high, Vthn, Vt_cell_pmos_high,
		   Vthp);
  Icell =
    cmos_ileakage (Wmemcellnmos, Wmemcellpmos, Vt_cell_nmos_high, Vthn,
  ...
  if (8 * B / BITOUT == 1 && Ndbl * Nspd == 1)
    {
      Cline =
	rows * (Cbitrow + Cbitmetal) + 2 * draincap (Wbitpreequ, PCH, 1);
	...
	draincap (Wiso, PCH, 1);
      Ccolmux =
	muxway * (draincap (Wiso, PCH, 1)) + gatecap (WsenseN + WsenseP,
						      10.0 / FUDGEFACTOR);
...
double
bitline_tag_delay (int C, int A, int B, int Ntwl, int Ntbl, int Ntspd,
		   ...
		   double *outrisetime, powerDef * power, double Tpre)
{
  ...
  if (Ntspd / Ntwl < 1)
    {				//Ntspd/Ntwl = Tag subarray columns / Tag array output width =
      ...
      Cline =
	rows * (Cbitrow + TagCbitmetal) + 2 * draincap (Wtbitpreequ, PCH, 1);
      Ccolmux = gatecap (WtsenseN + WtsenseP, 10.0 / FUDGEFACTOR);
      Rlineb = TagRbitmetal * rows / 2.0;
      r1 = Rlineb;
    }
  else
    {				//Tag array output width comes from one tag subarray
      Cline =
	rows * (Cbitrow + TagCbitmetal) + 2 * draincap (Wtbitpreequ, PCH,
							1) + draincap (Wtiso,
								       PCH,
								       1);
      Ccolmux =
	Ntspd / Ntwl * (draincap (Wtiso, PCH, 1)) + gatecap (WtsenseN +
							     WtsenseP,
							     10.0 /
							     FUDGEFACTOR);
      Rlineb = TagRbitmetal * rows / 2.0;
...
double
sense_amp_delay (int C, int B, int A, int Ndwl, int Ndbl, double Nspd,
		 double inrisetime, double *outrisetime, powerDef * power)
{
  double Cload;
  int cols, Nact;
  ...
  if (dualVt == 1)
    {
      IsenseEn =
	simplified_nmos_leakage (WsenseEn * inv_Leff, Vt_cell_nmos_high);
      IsenseN =
	simplified_nmos_leakage (WsenseN * inv_Leff, Vt_cell_nmos_high);
      IsenseP =
	simplified_pmos_leakage (WsenseP * inv_Leff, Vt_cell_pmos_high);
  ...
  Cload =
    gatecap (WsenseP + WsenseN, 5.0 / FUDGEFACTOR) + draincap (WsPch, PCH,
							       1) +
    gatecap (Woutdrvnandn + Woutdrvnandp,
	     1.0 / FUDGEFACTOR) + gatecap (Woutdrvnorn + Woutdrvnorp,
					   1.0 / FUDGEFACTOR);

  *outrisetime = tfalldata;
  power->readOp.dynamic =
    0.5 * Cload * VddPow * VddPow * BITOUT * A * muxover;
...
double
sense_amp_tag_delay (int C, int B, int A, int Ntwl, int Ntbl, int Ntspd,
		     double NSubbanks, double inrisetime, double *outrisetime,
		     powerDef * power)
{
  double Cload;
  ...
  if (dualVt == 1)
    {
      IsenseEn =
	simplified_nmos_leakage (WtsenseEn * inv_Leff, Vt_cell_nmos_high);
      IsenseN =
	simplified_nmos_leakage (WtsenseN * inv_Leff, Vt_cell_nmos_high);
      IsenseP =
	simplified_pmos_leakage (WtsenseP * inv_Leff, Vt_cell_pmos_high);
      IoBufN =
	simplified_nmos_leakage (WtoBufN * inv_Leff, Vt_cell_nmos_high);
      IoBufP =
	simplified_pmos_leakage (WtoBufP * inv_Leff, Vt_cell_pmos_high);
  ...
  Cload =
    gatecap (WtsenseP + WtsenseN, 5.0 / FUDGEFACTOR) + draincap (WtsPch, PCH,
								 1);
...
double
compare_time (int C, int A, int Ntbl, int Ntspd, double NSubbanks,
	      double inputtime, double *outputtime, powerDef * power)
{
  ...
  lkgCurrent =
    0.5 * A * cmos_ileakage (Wcompinvn1, Wcompinvp1, Vt_bit_nmos_low, Vthn,
			     Vt_bit_pmos_low, Vthp);
  /* Second Inverter */
  ...
  lkgCurrent +=
    0.5 * A * cmos_ileakage (Wcompinvn2, Wcompinvp2, Vt_bit_nmos_low, Vthn,
			     Vt_bit_pmos_low, Vthp);
  ...
  lkgCurrent +=
    0.5 * A * cmos_ileakage (Wcompinvn3, Wcompinvp3, Vt_bit_nmos_low, Vthn,
			     Vt_bit_pmos_low, Vthp);
  ...
  c1 = (tagbits) * (draincap (Wcompn, NCH, 1) + draincap (Wcompn, NCH, 2))
    + draincap (Wcompp, PCH, 1) + gatecap (WmuxdrvNANDn + WmuxdrvNANDp,
					   20.0 / FUDGEFACTOR) +
    cols * Cwordmetal;
  ...
  lkgCurrent +=
    0.5 * A * cmos_ileakage (Wevalinvn, Wevalinvp, Vt_bit_nmos_low, Vthn,
			     Vt_bit_pmos_low, Vthp);
  ...
  lkgCurrent +=
    2 * cols * 0.2 * 0.5 * A * cmos_ileakage (Wcompn, Wcompp, Vt_bit_nmos_low,
					      Vthn, Vt_bit_pmos_low, Vthp);
  // leakage due to the mux driver
  lkgCurrent +=
    0.5 * A * cmos_ileakage (Wmuxdrv12n, Wmuxdrv12p, Vt_bit_nmos_low, Vthn,
			     Vt_bit_pmos_low, Vthp);
  ...
  if ((tstep) <= (0.5 * (Vdd - Vt) / m))
    {
      ...
      c =
	-2 * (tstep) * (Vdd - Vt) + 1 / m * ((Vdd * VTHEVALINV) -
...
double
mux_driver_delay (int Ntbl, int Ntspd, double inputtime,
		  double *outputtime, double wirelength)
{
  ...
  Ceq =
    gatecap (Wmuxdrv3n + Wmuxdrv3p,
	     15.0 / FUDGEFACTOR) + 2 * draincap (WmuxdrvNORn, NCH,
						 1) + draincap (WmuxdrvNORp,
								PCH, 2);
  ...
  Ceq =
    BITOUT * gatecap (Woutdrvseln + Woutdrvselp + Woutdrvnorn + Woutdrvnorp,
		      20.0) + draincap (Wmuxdrv3p, PCH,
					1) + draincap (Wmuxdrv3n, NCH,
						       1) +
    GlobalCwordmetal * wirelength;
  Req =
    (GlobalRwordmetal * wirelength) / 2.0 + transreson (Wmuxdrv3p, PCH, 1);
  tf = Ceq * Req;
...
void
precalc_muxdrv_widths (int C, int B, int A, int Ndwl, int Ndbl, double Nspd,
		       double *wirelength_v, double *wirelength_h)
{
  int l_muxdrv_v = 0, l_muxdrv_h =
    0, cols_subarray, rows_subarray, nr_subarrays_left, horizontal_step,
    vertical_step, v_or_h;
  ...
  if (Ndwl * Ndbl == 1)
    {
      ...
      Cline =
	BITOUT * gatecap (Woutdrvseln + Woutdrvselp + Woutdrvnorn +
			  Woutdrvnorp,
			  20.0 / FUDGEFACTOR) +
...
double
mux_driver_delay_dualin (int C, int B, int A, int Ntbl, int Ntspd,
			 ...
			 powerDef * power)
{
  ...
  lkgCurrent =
    0.5 * 0.2 * muxover * cmos_ileakage (WmuxdrvNORn, WmuxdrvNORp,
					 Vt_bit_nmos_low, Vthn,
					 Vt_bit_pmos_low, Vthp);
  lkgCurrent +=
    0.5 * 0.2 * muxover * cmos_ileakage (WmuxdrvNANDn, WmuxdrvNANDp,
					 Vt_bit_nmos_low, Vthn,
					 Vt_bit_pmos_low, Vthp);
  ...
  lkgCurrent +=
    0.5 * muxover * cmos_ileakage (Wmuxdrv3n, Wmuxdrv3p, Vt_bit_nmos_low,
				   Vthn, Vt_bit_pmos_low, Vthp);
  ...
  if (nr_muxdrvtreesegments)
	...
	gatecap (3 * WmuxdrvtreeN[nr_muxdrvtreesegments - 1],
		 0) + Cmuxdrvtreesegments[nr_muxdrvtreesegments - 1];
      Req =
	transreson (Wmuxdrv3n, NCH,
		    1) + Rmuxdrvtreesegments[nr_muxdrvtreesegments - 1];
	...
	horowitz (inrisetime, tf, VTHINV360x240, VTHINV360x240, RISE);
      ...
      lkgCurrent +=
	A * 0.5 * cmos_ileakage (Wmuxdrv3n, Wmuxdrv3p, Vt_bit_nmos_low, Vthn,
				 Vt_bit_pmos_low, Vthp);
    }
  ...
  for (i = nr_muxdrvtreesegments; i > 2; i--)
    {
      /*dt: this too should alternate... */
      Ceq =
	(Cmuxdrvtreesegments[i - 2] +
	 draincap (2 * WmuxdrvtreeN[i - 1], PCH,
		   1) + draincap (WmuxdrvtreeN[i - 1], NCH,
				  1) + gatecap (3 * WmuxdrvtreeN[i - 2],
						0.0));
      Req =
	(Rmuxdrvtreesegments[i - 2] +
	 transreson (WmuxdrvtreeN[i - 1], NCH, 1));
      tf = Req * Ceq;
      /*dt: This shouldn't be all falling, but interleaved. Have to fix that at some point. */
      this_delay =
	horowitz (inrisetime, tf, VTHINV360x240, VTHINV360x240, RISE);
  ...
  if (nr_muxdrvtreesegments)
	...
	BITOUT * gatecap (Woutdrvseln + Woutdrvselp + Woutdrvnorn +
			  Woutdrvnorp,
			  20.0 / FUDGEFACTOR) + +Cdectreesegments[0] +
	draincap (2 * WmuxdrvtreeN[0], PCH, 1) + draincap (WmuxdrvtreeN[0],
							   NCH, 1);
      Rwire = Rmuxdrvtreesegments[0];
      tf = (Rwire + transreson (2 * WmuxdrvtreeN[0], PCH, 1)) * Ceq;
      this_delay =
	horowitz (inrisetime, tf, VTHINV360x240, VTHOUTDRINV, FALL);;
      Tst3 += this_delay;
      inrisetime = this_delay / VTHOUTDRINV;
      dynPower += A * Ceq * .5 * VddPow * VddPow;
      lkgCurrent +=
	pow (2,
	     nr_muxdrvtreesegments) * A * 0.5 *
	cmos_ileakage (WmuxdrvtreeN[0], 2 * WmuxdrvtreeN[0], Vt_bit_nmos_low,
		       Vthn, Vt_bit_pmos_low, Vthp);
	...
	BITOUT * gatecap (Woutdrvseln + Woutdrvselp + Woutdrvnorn +
      ...
      Req =
	(GlobalRwordmetal * wirelength_h +
	 GlobalRbitmetal * wirelength_v) / 2.0;
...
double
senseext_driver_delay (int A, char fullyassoc,
		       double inputtime, double *outputtime,
		       double wirelength_sense_v, double wirelength_sense_h,
		       powerDef * power)
{
  double Ceq, Req, tf, nextinputtime;
  //double Tst1,Tst2,Tst3;
  ...
  lkgCurrent =
    0.5 * BITOUT * A * muxover * cmos_ileakage (Wsenseextdrv1n,
						Wsenseextdrv1p,
						Vt_bit_nmos_low, Vthn,
						Vt_bit_pmos_low, Vthp);
  /* second driver stage */
  ...
  Req =
    (GlobalRwordmetal * wirelength_sense_h +
     GlobalRbitmetal * wirelength_sense_v) / 2.0 + transreson (Wsenseextdrv2p,
							       PCH, 1);
  ...
  *outputtime = Tst2 / (1 - VTHOUTDRNAND);
  ...
  lkgCurrent =
    0.5 * BITOUT * A * muxover * cmos_ileakage (Wsenseextdrv2n,
						Wsenseextdrv2p,
						Vt_bit_nmos_low, Vthn,
						Vt_bit_pmos_low, Vthp);
...
double
total_out_driver_delay (int C, int B, int A, char fullyassoc, int Ndbl,
			double Nspd, int Ndwl, int Ntbl, int Ntwl, int Ntspd,
			...
			double *outputtime, powerDef * power)
{
  ...
  int cols_data_subarray, rows_data_subarray, cols_tag_subarray,
    rows_tag_subarray;
  double subbank_v, subbank_h, sub_v = 0.0, sub_h =
    0.0, inter_v, inter_h, htree;
  int htree_int, tagbits;
  int cols_fa_subarray, rows_fa_subarray;
  ...
  if (!fullyassoc)
      ...
      if (!force_tag)
	{
	  ...
	  //tagbits = ADDRESS_BITS + EXTRA_TAG_BITS - (int) logtwo ((double) B);
	  tagbits =
	    (int) (ADDRESS_BITS + EXTRA_TAG_BITS - (int) logtwo ((double) B) +
		   EPSILON);
	}
      else
	{
...
double
valid_driver_delay (int C, int B, int A, char fullyassoc, int Ndbl, int Ndwl,
		    double Nspd, int Ntbl, int Ntwl, int Ntspd,
		    double *NSubbanks, double inputtime, powerDef * power)
{
  ...
  wire_cap =
    GlobalCbitmetal * (l_valdrv_v + subbank_v) +
    GlobalCwordmetal * (l_valdrv_h + subbank_h);
  wire_res =
    GlobalRwordmetal * (l_valdrv_h + subbank_h) * 0.5 +
...
double
half_compare_delay (int C, int B, int A, int Ntwl, int Ntbl, int Ntspd,
		    double NSubbanks, double inputtime, double *outputtime,
		    powerDef * power)
{
  double Req, Ceq, tf, st1del, st2del, st3del, nextinputtime, m;
  double c1, c2, r1, r2, tstep, a, b, c;
  ...
  double Wxtramuxdrv1n = 0, Wxtramuxdrv1p = 0, Wxtramuxdrv2n =
    0, Wxtramuxdrv2p = 0;
  double Cload = 0;
  ...
  if (!force_tag)
    {
      ...
      tagbits =
	(int) (ADDRESS_BITS - (int) logtwo ((double) C) +
	       (int) logtwo ((double) A) - (int) (logtwo (NSubbanks)) +
	       EPSILON);
  ...
  lkgCurrent =
    0.5 * A * cmos_ileakage (Wxtramuxdrv2n, Wxtramuxdrv2p, Vt_bit_nmos_low,
			     Vthn, Vt_bit_pmos_low, Vthp);
  ...
  *outputtime = muxdrv1del / VTHCOMPINV;
  ...
  lkgCurrent =
    0.5 * A * cmos_ileakage (Wxtramuxdrv1n, Wxtramuxdrv1p, Vt_bit_nmos_low,
			     Vthn, Vt_bit_pmos_low, Vthp);
...
double
dataoutput_delay (int C, int B, int A, char fullyassoc, int Ndbl, double Nspd,
		  ...
		  powerDef * power)
{
  ...
  lkgCurrent +=
    0.5 * BITOUT * routed_ways * muxover * cmos_ileakage (Woutdrvseln,
							  Woutdrvselp,
							  Vt_bit_nmos_low,
							  Vthn,
							  Vt_bit_pmos_low,
							  Vthp);
  lkgCurrent +=
    0.5 * 0.2 * BITOUT * routed_ways * muxover * cmos_ileakage (Woutdrvnorn,
								Woutdrvnorp,
								Vt_bit_nmos_low,
								Vthn,
								Vt_bit_pmos_low,
								Vthp);
  lkgCurrent +=
    0.5 * 0.2 * BITOUT * routed_ways * muxover * cmos_ileakage (Woutdrvnandn,
								Woutdrvnandp,
								Vt_bit_nmos_low,
								Vthn,
								Vt_bit_pmos_low,
  ...
  if (nr_outdrvtreesegments)
    {
      Ceq =
	(draincap (Woutdrivern, NCH, 1) +
	 draincap (Woutdriverp, PCH,
		   1)) * A * muxover + Coutdrvtreesegments[0] +
	gatecap (3 * WoutdrvtreeN[1], 0.0);
    }
  else
    {
      Ceq =
  ...
  lkgCurrent +=
    0.5 * BITOUT * routed_ways * muxover * cmos_ileakage (Woutdrivern,
							  Woutdriverp,
							  Vt_bit_nmos_low,
							  Vthn,
							  Vt_bit_pmos_low,
  ...
  *outrisetime = outdel / (1.0 - VTHOUTDRIVE);
  ...
  if (nr_outdrvtreesegments)
      ...
      for (i = 1; i < nr_outdrvtreesegments - 1; i++)
	{
	  /*dt: this too should alternate... */
	  Ceq =
	    (Coutdrvtreesegments[i] + draincap (2 * WoutdrvtreeN[i], PCH, 1) +
	     draincap (WoutdrvtreeN[i], NCH,
		       1) + gatecap (3 * WoutdrvtreeN[i], 0.0));
	  Req =
	    (Routdrvtreesegments[i] + transreson (WoutdrvtreeN[i], NCH, 1));
	  tf = Req * Ceq;
	  /*dt: This shouldn't be all falling, but interleaved. Have to fix that at some point. */
	  this_delay =
	  ...
	  *outrisetime = this_delay / (1.0 - VTHINV360x240);
	  ...
	  lkgCurrent +=
	    pow (2,
		 nr_outdrvtreesegments -
		 i) * routed_ways * BITOUT * 0.5 *
	    cmos_ileakage (WoutdrvtreeN[i], 2 * WoutdrvtreeN[i],
			   Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp);
	}
      Ceq =
	Coutdrvtreesegments[nr_outdrvtreesegments - 1] +
	draincap (2 * WoutdrvtreeN[nr_outdrvtreesegments - 1], PCH,
		  1) + draincap (WoutdrvtreeN[nr_outdrvtreesegments - 1], NCH,
				 1) + gatecap (Wsenseextdrv1n +
					       Wsenseextdrv1p,
					       10.0 / FUDGEFACTOR);
      Req =
	(Routdrvtreesegments[nr_outdrvtreesegments - 1] +
...
double
selb_delay_tag_path (double inrisetime, double *outrisetime, powerDef * power)
{
  double Ceq, Tst1, tf;
...
double
precharge_delay (double worddata)
{
...
void
calc_wire_parameters (parameter_type * parameters)
{
  ...
  TagCbitmetal =
    (BitHeight +
     2 * WIREPITCH * (parameters->num_read_ports +
		      parameters->num_readwrite_ports - 1)) * Default_Cmetal;
  /*dt: the +1 is because a RW port needs two bitlines, thus longer wordlines */
  TagCwordmetal =
    (BitWidth +
     WIREPITCH * (parameters->num_read_ports +
		  2 * (parameters->num_readwrite_ports -
		       1))) * Default_Cmetal;
  ...
  TagRbitmetal =
    (BitHeight +
     2 * WIREPITCH * (parameters->num_read_ports +
		      ...
		      1)) * (Default_CopperSheetResistancePerMicroM /
			     WIREHEIGTHRATIO * FUDGEFACTOR * FUDGEFACTOR);
  TagRwordmetal =
    (BitWidth +
     WIREPITCH * (parameters->num_read_ports +
		  2 * (parameters->num_readwrite_ports -
...
void
calculate_time (result_type * result, arearesult_type * arearesult,
		...
		double *NSubbanks)
{
  ...
  powerDef compare_tag_power, selb_power, mux_driver_power,
    valid_driver_power;
  ...
  for (counter = 0; counter < 2; counter++)
      ...
      if (!parameters->fully_assoc)
	{
	  ...
	  for (Nspd =
	       (double) (BITOUT) / (double) (parameters->block_size * 8);
		  ...
		  for (Ndbl = 1; Ndbl <= MAXDATAN; Ndbl = Ndbl * 2)
			   ...
			   parameters->fully_assoc, (*NSubbanks)))
			{
			  ...
			  if (8 * parameters->block_size /
				...
				{
				  if (8 * parameters->block_size *
				      Nspd / BITOUT > MAX_COL_MUX)
				    {
				      muxover =
					(8 * parameters->block_size /
					 BITOUT) / (MAX_COL_MUX / (Nspd));
				    }
				  else
				    {
				      muxover = 1;
			  ...
			  if (*NSubbanks == 1.0)
				...
				decoder_delay (parameters->cache_size,
			      ...
			      wordline_data =
				wordline_delay (parameters->cache_size,
						parameters->block_size,
						parameters->
						data_associativity, Ndwl,
					   ...
					   (*NSubbanks)))
					{
					  ...
					  if (*NSubbanks > 1)
						...
						decoder_delay (parameters->
					      ...
					      Tpre = decoder_data;
					      max_delay =
						MAX (max_delay, decoder_data);
					      inrisetime = outrisetime;

					      reset_powerDef
						(&wordline_data_power);
					      wordline_data =
						wordline_delay (parameters->
								cache_size,
								parameters->
								block_size,
								parameters->
						...
						bitline_delay (parameters->
					      ...
					      inrisetime = outrisetime;
					      max_delay =
						MAX (max_delay, bitline_data);


					      reset_powerDef
						(&sense_amp_data_power);
					      sense_amp_data =
						sense_amp_delay (parameters->
								 cache_size,
								 parameters->
								 block_size,
								 parameters->
					  ...
					  if (tag_iteration_counter <
					      MAX_CACHE_ENTRIES
					      &&
					      valid_tag_cache_entries
					      [tag_iteration_counter] == TRUE)
					    {
					      current_cache_entry =
						cached_tag_calculations
						[tag_iteration_counter];
					      decoder_tag =
						current_cache_entry.decoder.
						delay;
					      copy_powerDef
						(&decoder_tag_power,
						 current_cache_entry.decoder.
					  ...
					  max_delay =
					    MAX (max_delay, sense_amp_tag);
					  inrisetime = outrisetime;

					  wirelength_v = wirelength_h = 0;
					  if (parameters->tag_associativity !=
					      1)
					    {
					  ...
					  if (parameters->tag_associativity ==
						...
						(parameters->cache_size,
						 ...
						 tag_associativity,
						 parameters->fully_assoc,
						 Ndbl, Ndwl, Nspd, Ntbl, Ntwl,
						 Ntspd, NSubbanks, inrisetime,
						 &valid_driver_power);
					  ...
					  total_power.readOp.dynamic +=
					    decoder_data_power.readOp.
					    dynamic * allreadports;
					  total_power.writeOp.dynamic +=
					    decoder_data_power.writeOp.
					    dynamic * allwriteports;
					  //mac_powerDef(&total_power,&wordline_data_power,allports);
					  total_power.readOp.dynamic +=
					    wordline_data_power.readOp.
					    dynamic * allreadports;
					  total_power.writeOp.dynamic +=
					    wordline_data_power.writeOp.
					    dynamic * allwriteports;
					  /*dt: We can have different numbers of read/write ports, so the power numbers have to keep that in mind. Ports which can do both reads and
					     ...
					     than senseamp power) */
					  total_power.readOp.dynamic +=
					    bitline_data_power.readOp.
					    dynamic * allreadports;
					  total_power.writeOp.dynamic +=
					    bitline_data_power.writeOp.
					    dynamic * allwriteports;
					  ...
					  total_power_without_routing.readOp.
					    dynamic *= (*NSubbanks);
					  total_power_without_routing.readOp.
					    leakage *= (*NSubbanks);
					  total_power_without_routing.writeOp.
					    dynamic *= (*NSubbanks);
					  total_power_without_routing.writeOp.
					    leakage *= (*NSubbanks);
					  ...
					  if (counter == 1)
					    {
					      if (objective_function (1, 1, 1, result->access_time / result->max_access_time, 1.0 / arearesult->efficiency, ((result->total_power.readOp.dynamic / result->cycle_time) + result->total_power.readOp.leakage + (result->total_power.writeOp.dynamic / result->cycle_time) + result->total_power.writeOp.leakage) / 2.0 / result->max_power) > objective_function (1, 1, 1, access_time / (result->max_access_time), 1.0 / efficiency, ((total_power.readOp.dynamic / (cycle_time)) + total_power.readOp.leakage + (total_power.writeOp.dynamic / (cycle_time)) + total_power.writeOp.leakage) / 2.0 / result->max_power))	//read and write dynamic energy components changed to power components by Shyam. Earlier
						//read and write dynamic energies were being added to leakage power components and getting swamped
						{
						  result->senseext_scale =
						    senseext_scale;
						  copy_powerDef (&result->
								 total_power,
								 total_power);
						  copy_powerDef (&result->
								 total_power_without_routing,
						  ...
						  copy_powerDef (&result->
								 decoder_power_tag,
								 decoder_tag_power);
						  result->dec_tag_driver =
						    decoder_tag_driver;
						  result->dec_tag_3to8 =
						    decoder_tag_3to8;
						  result->dec_tag_inv =
						    decoder_tag_inv;
						  result->dec_data_driver =
						    decoder_data_driver;
						  result->dec_data_3to8 =
						    decoder_data_3to8;
						  result->dec_data_inv =
						    decoder_data_inv;
						  result->
						  ...
						  copy_powerDef (&result->
								 ...
								 bitline_tag_power);
						  result->
						    sense_amp_delay_data =
						    sense_amp_data;
						  copy_powerDef (&result->
								 sense_amp_power_data,
								 sense_amp_data_power);
						  result->
						    sense_amp_delay_tag =
						    sense_amp_tag;
						  copy_powerDef (&result->
								 ...
								 sense_amp_tag_power);
						  result->
						    total_out_driver_delay_data
						    = total_out_driver;
						  copy_powerDef (&result->
								 total_out_driver_power_data,
								 total_out_driver_power);
					    ...
					    {
					      if (result->max_access_time <
						  access_time)
						{
						  result->max_access_time =
						    access_time;
						}
					      if (result->max_power <
						  ((total_power.readOp.
      ...
      else
	{
	  ...
	  for (Ndbl = 1; Ndbl <= MAXDATAN; Ndbl = Ndbl * 2)
		   ...
		   Ndbl, Nspd, parameters->fully_assoc, (*NSubbanks)))
		{
		  ...
		  colsfa =
		    CHUNKSIZE * parameters->block_size * 1 * Nspd / Ndwl;
		  ...
		  Clinefa =
		    (gatecappass (Wmemcella, (BitWidth - 2 * Wmemcella) / 2.0)
		     ...
		     Cwordmetal) * colsfa;
		  Rpdrivefa =
		    desiredrisetimefa / (Clinefa * log (VSINV) * -1.0);
		  ...
		  wordline_data = wordline_delay (parameters->cache_size,
						  parameters->block_size,
						  1, Ndwl, Ndbl, Nspd,
						  inrisetime, &outrisetime,
						  &wordline_data_power);
...
*/
		  ...
		  //mac_powerDef(&total_power,&decoder_data_power,allports);
		  total_power.readOp.dynamic +=
		    decoder_data_power.readOp.dynamic * allreadports;
		  total_power.writeOp.dynamic +=
		    decoder_data_power.writeOp.dynamic * allwriteports;
		  //mac_powerDef(&total_power,&wordline_data_power,allports);
		  total_power.readOp.dynamic +=
		    wordline_data_power.readOp.dynamic * allreadports;
		  total_power.writeOp.dynamic +=
		    wordline_data_power.writeOp.dynamic * allwriteports;
		  ...
		  if (counter == 1)
			...
			result->max_access_time = access_time;
			  ...
			  ((total_power.readOp.dynamic / (cycle_time)) +
			   ...
			   total_power.writeOp.leakage) / 2.0)
			{
			  result->max_power =
			    ((total_power.readOp.dynamic / (cycle_time)) +
			     total_power.readOp.leakage +
