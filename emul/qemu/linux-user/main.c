...
uint32_t  qemuesesc_getCP15tls2(void * env){
	return  (( (((CPUARMState *)env)->cp15.c13_tls2)));
}
#endif
...
void cpu_loop(CPUARMState *env)
{
    ...
    CPUARMState *next_env =  env;
    for(;;) {
        /* next env could have changed */
        env = next_env;
        cpu_exec_start(env);
        ...
        trapnr = cpu_arm_exec(env, &next_env);
...
#if defined (CONFIG_ESESC_system) || defined (CONFIG_ESESC_user)
uint64_t qemuesesc_getReg(void * env, uint8_t reg){
	return (((CPUSPARCState *)env)->regbase[get_reg_index((CPUSPARCState *)env, ((CPUSPARCState *)env)->cwp, 6)]);
}
#endif
...
#endif
    printf("MCore at 0x%lx and PCore at 0x%lx\n", (long unsigned int)env, (long unsigned int)penv);
    env->IsPCore  = 0;
    env->PCore    = penv;
    penv->MCore   = env;
    env->next_cpu = 0;
