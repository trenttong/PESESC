...
void cpu_reset(CPUARMState *env)
{
    ...
    // NOTE: It gets overwritten by cpu_copy for threads 1..n
    env->op_cnt  = 0;
    ...
    env->xtrace_op_tp = env->xtrace_op_tp_raw;
    env->xtrace_xp_op_tp = env->xtrace_xp_op_tp_raw;
#endif
#if defined (CONFIG_USER_ONLY)
...
CPUARMState *cpu_arm_init(const char *cpu_model)
{
    ...
    if (tcg_enabled() && !inited) {
        ...
        arm_translate_init();
#if defined (CONFIG_ESESC_system) || defined (CONFIG_ESESC_user)
        arm_translate_init_esescqueue(0); // assuming the first thread always has fid = 0
				                                  // The other threads will be initialized at the clone syscall
#endif
    }
