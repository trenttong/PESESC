...
/* 16 GPR */
#define ARM_INVALID_REG  16
#define INVALID_REG ARM_INVALID_REG
#define ARM_INVALID_MEMADD ((unsigned) -1)
#define INVALID_MEMADD  ARM_INVALID_MEMADD
#define IS_ARM_ARCH_VALID_REG(X) (X<ARM_INVALID_REG)
#define IS_VALID_ARCH_REG(X)  IS_ARM_ARCH_VALID_REG(X)
#define IS_VALID_OR_NULL_ARCH_REG(X)  (IS_ARM_ARCH_VALID_REG(X) || ARM_INVALID_REG==X)

/* max number of memory accesses per instruction */
#define MAX_MEM_PER_INSN 16
#define MAX_REG_PER_INSN 16


/// disble one of the following to disable respective tracing.
#define TRACE_PRINT_VERIFY         0
#define ENABLE_INSTRUCTION_TRACING 1 
#define ENABLE_MEMORY_TRACING      1 
#define ENABLE_REGISTER_TRACING    1 
#define ENABLE_BRANCH_TRACING      1 
...
typedef struct MemoryByte {
   struct MemoryByte *prev;
   char               data;
   uint32_t           addr;
   struct MemoryByte *next;
} MemoryByte;


/* this is the # of instrucction to execute before core switching 
...
typedef struct CPUARMState {
...
#endif
#if defined (CONFIG_ESESC_system) || defined (CONFIG_ESESC_user)
    uint32_t driving_processor;

    uint32_t xtrace_op_cnt; // number of uops executed.
    CPUTracePoint  xtrace_op_tp_raw[128+CF_COUNT_MASK];
    CPUTracePoint* xtrace_op_tp;

    uint32_t op_cnt; // number of uops executed
    ExpandedCPUTracePoint  xtrace_xp_op_tp_raw[128+CF_COUNT_MASK];
    ExpandedCPUTracePoint* xtrace_xp_op_tp;

    /// --------------------------------------------------- ///
    /* instrumentation buffers */
    /// --------------------------------------------------- ///
    target_ulong insn_cnt;
    ...
    uint32_t st_linear;
    uint32_t ld_linear;
    uint8_t  st_databytes[8];
    uint8_t  ld_databytes[8];
    uint32_t blinear;
    uint8_t  bdata;
    ...
    /* ATTA: defined a variable to count the number of prefetches that have been
    generated by the P-core and not yet consumed by the main core. */
    long long int prefetchDistance;
    long long int maxPrefetchDistance;
} CPUARMState;
...
#endif
int cpu_arm_exec(CPUARMState *s, CPUARMState **n);
int cpu_arm_assist_exec(CPUARMState *s);
void cpu_arm_close(CPUARMState *s);
void do_interrupt(CPUARMState *);
void switch_mode(CPUARMState *, int);
uint32_t do_arm_semihosting(CPUARMState *env);
bool tis_driving_processor(CPUARMState *);
void set_driving_processor(CPUARMState *);
