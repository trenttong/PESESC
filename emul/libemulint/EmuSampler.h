...
public:
  ...
  virtual void queue(uint32_t insn, uint64_t pc, uint64_t addr, uint64_t data, uint32_t fid, char op, uint64_t icount, void *env,
						...
						uint32_t target/// target of branch
  	  	  	  	  	  ){ fprintf(stderr, "OOPS: you're calling an unimplemented EmuSampler::queue() function!\n"); }
