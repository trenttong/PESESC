...
void TaskHandler::report(const char *str) {
  ...
  DelinquencyAnalyzer::sortAndReportTopDelinquentPCs();
  ...
  /* main simulation loop {{{1 */
{
  while(!terminate_all) {
    for(size_t i =0;i<running_size;i++) {
      ...
      simu->fetch(fid);
      simu->execute();
#ifdef PRINT_CLOCKTICKS
      if(fid == 0 && simu->getClockTicks() % 1000 == 0)
    	  printf("\rP(0):clockTicks = %lld\t", simu->getClockTicks());
#endif // PRINT_CLOCKTICKS
#ifdef CREATE_PCORE
      GProcessor *psimu = allmaps[fid].psimu;
      psimu->fetch(fid+1);
      psimu->execute();
#endif // CREATE_PCORE
...
#ifdef CREATE_PCORE
    allmaps[i].psimu->setEmulInterface(emulas[i]);
#endif // CREATE_PCORE
...
void TaskHandler::unplug() 
  /* delete objects {{{1 */
{
#if 0	// ATTA: enabled this to dump pslice output
  for(size_t i=0; i<cpus.size() ; i++) {
    if(cpus[i]) delete cpus[i];
  }
#endif
#if 1	// ATTA: enabled this to dump pslice output
  for(size_t i=0; i<emulas.size() ; i++) {
    if(emulas[i]) delete emulas[i];
  }
#endif
}
/* }}} */
